...
import com.hawesome.bleconfig.ext.has
import com.hawesome.bleconfig.ext.toIntList
...
/*
*   蓝牙通信指令
*   帮助文档：https://github.com/Jasonchenlijian/FastBle/wiki
* */
...
object BluetoothKit {
    ...
    * */
    fun getDeviceType(bleName: String) = bleName.split('/').first()
    fun getDeviceSerial(bleName: String) = bleName.split('/').last()
    ...
    private fun updateUUID(gatt: BluetoothGatt?) {
        gatt?.services?.firstOrNull { it.uuid has SERVICE_UUID }?.let { service ->
        ...
        bleManager.notify(
            ...
            object : BleNotifyCallback() {
                ...
                override fun onCharacteristicChanged(data: ByteArray?) {
                    ...
                    //因需在设置、核对最后一个片区时触发特定事件，故通信失败时必须马上重试，保证通信片区序列正确
                    if (bleCMD0.repeat == 0) {
                        bleCMDList.removeAt(0)
                    }
                    if (bleCMDList.size > 0) {
                        executeNextCMD()
                    }
